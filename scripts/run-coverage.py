#!/usr/bin/env python3
"""
Kappa-Core Test Coverage Script
...
def configure_cmake(
    project_root: Path, build_dir: Path, vcpkg_toolchain: Path, generator: str = "Ninja"
):
...
def clean_coverage_data(build_dir: Path):
    ...
    print_step("Running kappa-coverage-clean target...")

    cmd = ["cmake", "--build", str(build_dir), "--target", "kappa-coverage-clean"]
...
def run_coverage(build_dir: Path):
    ...
    cmd = ["cmake", "--build", str(build_dir), "--target", "kappa-coverage-html"]
...
def display_summary(build_dir: Path):
    ...
    test_exe = build_dir / "tests" / "TestKappaCore"
    if platform.system() == "Windows":
...
def print_results(build_dir: Path, open_browser: bool):
    ...
    print(f"  {Colors.GREEN}•{Colors.NC} cmake --build build --target kappa-coverage-report    (text report)")
    print(
        f"  {Colors.GREEN}•{Colors.NC} cmake --build build --target kappa-coverage-summary   (brief summary)"
    )
    print(
        f"  {Colors.GREEN}•{Colors.NC} cmake --build build --target kappa-coverage-clean     (clean coverage data)"
    )
    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run test coverage analysis for Kappa-Core",
...
Examples:
  ...
  python scripts/run-coverage.py --no-build --open  # Regenerate report only
    ...
    print()
    print(f"{Colors.CYAN}{Colors.BOLD}Kappa-Core Coverage Analysis{Colors.NC}")
    ...
    if not args.no_build:
        configure_cmake(project_root, build_dir, env["vcpkg_toolchain"], generator=args.generator)
